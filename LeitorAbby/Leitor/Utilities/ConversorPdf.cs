...
namespace Leitor.Utilities
{
    public static class ConversorPdf
    {
        ...
        public static string ExtrairTexto(string nomeArquivo)
        {
            ...
            if (String.IsNullOrWhiteSpace(texto) || texto.Length < 10 || texto.Contains("GOIÂNIA") || texto.Contains("PREFEITURA MUNICIPAL DE MOGI MIRIM") || texto.Contains("Prefeitura Municipal  de Taubaté") || texto.Contains("PREFEITURA DA ESTÂNCIA DE ATIBAIA") || texto.Contains("Munic355pio de Presta347343o do Servi347o"))
            {
                texto = PdfToText.ExtrairTextoDoPdf(nomeArquivo);
            }
            if (texto.Contains("PREFEITURA MUNICIPAL DE ARAÇATUBA"))
                texto = texto.Replace("•", "");
            return texto.Trim();
        }
